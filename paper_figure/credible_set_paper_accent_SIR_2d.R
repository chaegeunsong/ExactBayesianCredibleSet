...
###############################################-
##  SIR on training to get 2d               ####
###############################################-
## beta
beta <- sir(x,y,3,4,"categorical")
dim(beta)
...
plot(squaregrid,squaregrid,pch=" ",
     xlab="first SIR direction", ylab="second SIR direction",
     asp = 1)
points(pred[y==1,1],pred[y==1,2],col="red")
points(pred[y==2,1],pred[y==2,2],col="green")
points(pred[y==3,1],pred[y==3,2],col="blue")
...
arrange_colorset <- c('blue','red','black','green','orange','purple')
arrange_phi_vec <- phi_vec[,c(3,1,4,2,5,6)]
flowerplot2d(predy,(1:n)[y==1],arrange_phi_vec,r=0.25,
             'red',arrange_colorset)
flowerplot2d(predy,(1:n)[y==2],arrange_phi_vec,r=0.25,
             'green',arrange_colorset)
flowerplot2d(predy,(1:n)[y==3],arrange_phi_vec,r=0.25,
             'blue',arrange_colorset)
flowerplot2d(predy,(1:n)[y==4],arrange_phi_vec,r=0.25,
             'black',arrange_colorset)
flowerplot2d(predy,(1:n)[y==5],arrange_phi_vec,r=0.25,
             'orange',arrange_colorset)
flowerplot2d(predy,(1:n)[y==6],arrange_phi_vec,r=0.25,
...
v5 <- split(datav, datav$v)[[4]]
v6 <- split(datav, datav$v)[[5]]
...
squaregrid <- c(min(range(pred[,1]), range(pred[,2])), 
                max(range(pred[,1]), range(pred[,2])))

#5.5/22.27672
cairo_ps("accent_SIR.eps", family = "Times New Roman")
par(family = "Times New Roman", mar = c(5, 6, 4, 2) + 0.1)
plot(c(-3,2.5),c(-3,2.5),pch=" ",
...
arrange_colorset <- c('blue','red','purple','green','orange','black')
arrange_phi_vec <- phi_vec[,c(3,1,6,2,5,4)]
flowerplot2d(predy,(1:n)[v==1][n1],arrange_phi_vec,r=0.247,
             'red',arrange_colorset, cex=1.25, ir=0.06)
flowerplot2d(predy,(1:n)[v==2][n2],arrange_phi_vec,r=0.247,
             'green',arrange_colorset, cex=1.25, ir=0.06)
flowerplot2d(predy,(1:n)[v==3][n3],arrange_phi_vec,r=0.247,
             'blue',arrange_colorset, cex=1.25, ir=0.06)
flowerplot2d(predy,(1:n)[v==4][n4],arrange_phi_vec,r=0.247,
             'black',arrange_colorset, cex=1.25, ir=0.06)
flowerplot2d(predy,(1:n)[v==5][n5],arrange_phi_vec,r=0.247,
             'orange',arrange_colorset, cex=1.25, ir=0.06)
flowerplot2d(predy,(1:n)[v==6][n6],arrange_phi_vec,r=0.247,
